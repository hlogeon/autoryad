...
class Models_Catalog {
  ...
  public function getList($countRows = 20, $mgadmin = false, $onlyActive = false) {
    ...
    $sql .= 'SELECT
      ...
      $this->currentCategory['id'] =  $this->currentCategory['id']?$this->currentCategory['id']:0;
        ...
        if (MG::getOption('productInSubcat')=='true') {       
          $filter = '(c.id IN (' .DB::quote( implode(',', $this->categoryId),1) . ') '
  ...
  public function exportToCsv($listProductId=array()) {
    ...
    if(empty($listProductId)){
      $maxCountPage = ceil($product->getProductsCount() / 1000);    
    }else{
      $maxCountPage = ceil(count($listProductId) / 1000);    
    }    
    ...
    for ($page = 1; $page <= $maxCountPage; $page++) {
      ...
      if(empty($listProductId)){
        $catalog->getList(1000, true);
      }else{
        $catalog->getListByUserFilter(1000, ' p.id IN ('.DB::quote($listId,1).')');        
      }
  ...
  public function addToCsvLine($row, $variant = false) {
    ...
    $row['old_price'] = '"' . str_replace("\"", "\"\"", MG::numberDeFormat($row['old_price'])) . '"';
